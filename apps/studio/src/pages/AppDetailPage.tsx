...
import { useBuilder } from '../components/PageBuilderContext';
import { FormLayoutProvider, FormLayoutInspector, FieldTypeIcon, useFormLayout } from '../components/formlayout';
...
/* ── Builder layout-mode panes — swap designer panes for the form layout editor ── */

/** Wraps the builder's StudioLayout in the form-layout engine while Layout mode edits a focused collection's form. */
function BuilderFormLayout({ token, children }: { token: string; children: ReactNode }) {
...
function BuilderLeftPane({ tree }: { tree: MenuNode[] }) {
...
function toMenuTree(nodes: ApiMenuNode[], parentId: string | null): MenuNode[] {
	return nodes.map((n) => ({
		id: n.id,
		label: n.label,
		label_my: n.label_my,
