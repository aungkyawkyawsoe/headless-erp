...
function useMessageScrollerController({
	...
	// Reset the “applied” flag when the prop changes so the new position is
	// applied again. Runs before the apply layout effect below (declaration
	// order), so the flag is fresh when that effect reads it.
	...
	const reconcileFollowMode = React.useCallback(
		(scrollable: MessageScrollerScrollable) => {
			...
			if (autoScrollRef.current && !scrollable.end && modeRef.current !== 'settling-jump' && modeRef.current !== 'anchored-to-message') {
		...
		[autoScrollRef, autoscrollingRef, lastScrollTopRef, modeRef, viewportRef],
	);
	...
	}, [reconcileFollowMode, stateStore, writeStateAttributes, contentRef, modeRef, scrollEdgeThresholdRef, spacerRef, viewportRef]);
	...
	const { flushPendingScrollToMessage, reanchorToAnchoredMessage, scrollToElement, scrollToEnd, scrollToMessage, scrollToStart } =
	...
	const restorePrependedAnchor = React.useCallback(() => {
		...
		return true;
	}, [scheduleStateCommit, scheduleVisibilitySync, prependRestoreRef, viewportRef]);
	...
	const applyDefaultScrollPosition = React.useCallback(() => {
		...
		if (defaultScrollPosition === 'last-anchor') {
			const content = contentRef.current;
			const viewport = viewportRef.current;
			const anchor = content && viewport ? getLastScrollAnchor(getMessageScrollerItems(content, spacerRef.current)) : null;
		...
		const reconcileScrollPosition = () => {
			...
			if (items.length > previousItemCount) {
				...
				if (anchor) {
					...
					if (autoScrollRef.current && modeRef.current === 'following-bottom' && hasMultipleNewScrollAnchors(items, previousItemCount)) {
						scrollToEnd({ behavior: 'auto' });
						return;
					}
	...
	const handleResize = React.useCallback(() => {
		...
		scheduleStateCommit();
		scheduleVisibilitySync();
	}, [reanchorToAnchoredMessage, scheduleStateCommit, scheduleVisibilitySync, scrollToEnd, autoScrollRef, modeRef, spacerHeightRef]);
	...
	}, [visibilityStore, visibilityFrameRef, visibilityObserverRef, visibleMessageIdsRef]);
	...
	const userScrollIntent = React.useCallback(() => {
		if (modeRef.current === 'following-bottom' || modeRef.current === 'anchored-to-message' || modeRef.current === 'settling-jump') {
	...
	}, [autoscrollingTimeoutRef, pendingScrollFrameRef, stateFrameRef, visibilityFrameRef, visibilityObserverRef]);
	...
	React.useLayoutEffect(() => {
		...
		commitScrollState();
	}, [autoScroll, commitScrollState, scrollToEnd, itemCountRef, modeRef]);
