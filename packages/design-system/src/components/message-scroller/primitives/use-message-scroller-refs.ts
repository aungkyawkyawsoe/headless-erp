...
function useMessageScrollerRefs({
	...
	// The two external stores are constructed once per instance (lazy state
	// initializer) and fan out via useSyncExternalStore.
	const [stateStore] = React.useState(() => createMessageScrollerStore(EMPTY_MESSAGE_SCROLLER_SCROLLABLE, areScrollStatesEqual));
	const [visibilityStore] = React.useState(() => createMessageScrollerVisibilityStore());

	// Track the latest prop values after each commit so callbacks read fresh
	// values without being recreated (the useLatest pattern).
	React.useEffect(() => {
