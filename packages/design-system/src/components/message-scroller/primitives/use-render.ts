// Vendored from @shadcn/react use-render (MIT).
...
function useRender<TElement extends React.ElementType, TState extends RenderState = RenderState>({
	defaultTagName,
	props,
	render,
	state = {} as TState,
	...
	const propsWithRef = {
		...
		ref: composeRefs(elementProps.ref as React.Ref<unknown> | undefined, renderProps.ref as React.Ref<unknown> | undefined),
	};

	return React.cloneElement(render, propsWithRef as React.Attributes & Record<string, unknown>);
}

function mergeProps<TElement extends React.ElementType>(
	...sources: Array<React.ComponentPropsWithRef<TElement> | Record<string, unknown> | undefined>
) {
...
function composeRefs<T>(...refs: Array<React.Ref<T> | undefined>): React.RefCallback<T> | undefined {
