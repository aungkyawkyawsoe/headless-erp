...
import { EMPTY_MESSAGE_SCROLLER_SCROLLABLE, EMPTY_MESSAGE_SCROLLER_VISIBILITY_STATE, SCROLL_POSITION_EPSILON } from './types';
...
function getMessageScrollerVisibilityState({
	...
	for (const item of getMessageScrollerItems(content, spacer)) {
		...
		const isVisible = trackByLayout && rect ? rect.bottom > lineTop && rect.top < viewportRect.bottom : visibleMessageIds.has(messageId);
...
function getContentBottom({ content, spacer, viewport }: { content: HTMLElement; spacer: HTMLElement | null; viewport: HTMLElement }) {
