...
const SIDEBAR_WIDTH_MIN_PX = 192; // 12rem — narrowest sensible expanded width
const SIDEBAR_WIDTH_MAX_PX = 480; // 30rem — widest sensible expanded width
const SIDEBAR_WIDTH_STORAGE_KEY = 'sidebar_width';
const SIDEBAR_RESIZE_THRESHOLD = 4; // px of pointer movement that counts as a drag
...
type SidebarContextProps = {
	...
	isResizing: boolean;
	/** Start a resize drag from a pointer down on the sidebar rail. */
...
function SidebarProvider({
	...
	// Expanded sidebar width — a CSS length string. When `resizable` is set,
	// it starts from the persisted value (or the default) and is updated by
	// dragging the sidebar rail. Kept in a ref so the pointer-up handler can
	// persist the final value without re-subscribing listeners.
	const [sidebarWidth, setSidebarWidth] = React.useState<string>(() => {
		if (typeof window === 'undefined') return SIDEBAR_WIDTH;
		try {
			return window.localStorage.getItem(SIDEBAR_WIDTH_STORAGE_KEY) ?? SIDEBAR_WIDTH;
		} catch {
	...
	// Handlers of an in-flight drag, so they can be cleaned up on unmount.
	...
	const persistWidth = React.useCallback((width: string) => {
		try {
			...
			// localStorage can be unavailable (e.g. sandboxed storybook iframes).
		}
	}, []);
	...
	const startSidebarResize = React.useCallback(
		(event: React.PointerEvent) => {
			if (!resizable) return;
			const container = (event.currentTarget as HTMLElement).closest("[data-slot='sidebar-container']") as HTMLElement | null;
			if (!container) return;

			event.preventDefault();
			const side: 'left' | 'right' = container.dataset.side === 'right' ? 'right' : 'left';
			const rect = container.getBoundingClientRect();
			const maxPx = Math.min(SIDEBAR_WIDTH_MAX_PX, Math.round(window.innerWidth * 0.6));

			const onMove = (moveEvent: PointerEvent) => {
				const rawWidth = side === 'right' ? rect.right - moveEvent.clientX : moveEvent.clientX - rect.left;
				const width = Math.round(Math.min(Math.max(rawWidth, SIDEBAR_WIDTH_MIN_PX), maxPx));
				const widthPx = `${width}px`;
				setSidebarWidth(widthPx);
				sidebarWidthRef.current = widthPx;
			};
		...
		[persistWidth, resizable],
	);

	// Safety net: drop listeners if the provider unmounts mid-drag.
	React.useEffect(() => {
		return () => {
		...
		<SidebarContext.Provider value={contextValue}>
			<div
				...
				className={cn(
					...
					// Drop the width transitions while dragging so the sidebar tracks
					// the pointer instead of lagging behind it.
					'data-[resizing=true]:[&_[data-slot=sidebar-container]]:transition-none data-[resizing=true]:[&_[data-slot=sidebar-gap]]:transition-none',
					className,
				)}
				{...props}
			>
	...
	return (
		<button
			...
			onPointerDown={(event) => {
				...
				// Detect whether the pointer actually moved; if so, the click that
				// follows should resize, not toggle.
				const onUp = (upEvent: PointerEvent) => {
					window.removeEventListener('pointerup', onUp);
					if (
						Math.abs(upEvent.clientX - startX) >= SIDEBAR_RESIZE_THRESHOLD ||
						Math.abs(upEvent.clientY - startY) >= SIDEBAR_RESIZE_THRESHOLD
					) {
			...
			className={cn(
				...
				// While resizable, the rail is a drag target: keep the browser from
				// scrolling/selecting mid-drag and highlight the grip while resizing.
				resizable && 'touch-none data-[resizing=true]:after:w-1 data-[resizing=true]:after:bg-ring',
				className,
			)}
			{...props}
		/>
...
function SidebarInput({ className, ...props }: React.ComponentProps<typeof Input>) {
	return <Input data-slot="sidebar-input" data-sidebar="input" className={cn('shadow-none', className)} {...props} />;
}
