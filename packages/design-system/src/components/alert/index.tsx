	...
	"group/alert relative grid w-full gap-0.5 rounded-lg border px-2.5 py-2 text-left text-sm has-data-[slot=alert-action]:relative has-data-[slot=alert-action]:pr-18 has-[>svg]:grid-cols-[auto_1fr] has-[>svg]:gap-x-2 *:[svg]:row-span-2 *:[svg]:translate-y-0.5 *:[svg:not([class*='size-'])]:size-4",
	{
		variants: {
			variant: {
				...
				info: 'bg-card text-info *:data-[slot=alert-description]:text-info/90',
				success: 'bg-card text-success *:data-[slot=alert-description]:text-success/90',
				warning: 'bg-card text-warning *:data-[slot=alert-description]:text-warning/90',
				invert: 'border-0 bg-invert text-invert-foreground *:data-[slot=alert-description]:text-invert-foreground/70',
			},
		},
		defaultVariants: {
			variant: 'default',
