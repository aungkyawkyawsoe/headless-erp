...
// Local secret files are regenerated by `writeAdminEnv` — never inherit the
// template author's `.dev.vars` / `.env.local` (git-ignored, but a copy would
// leak their JWT_SECRET / admin password into the new project).
const EXCLUDED_FILES = new Set(['.DS_Store', 'tsconfig.tsbuildinfo', '*.tsbuildinfo', '.dev.vars', '.env.local']);
...
function personalizeProject(destRoot: string, projectName: string): void {
	...
	if (pkgRaw) {
		try {
			...
			// Always adopt the project name — the template ships its OWN name
			// (e.g. the starter's), which would otherwise linger as the package id.
 ...
 *
 * The repo's SSOT rule is `infra/env.prod` → generated wrangler jsonc
 * (`scripts/gen-wrangler.mjs`, CI-gated by `pnpm check:infra`). So when the
 * scaffold ships that pipeline we personalize the SSOT and REGENERATE — never
 * hand-edit the generated configs (doing so desynced them and made
...
function writeWranglerConfig(destRoot: string, projectName: string, dbName: string, adminEmail: string): void {
	...
	if (readFile(envPath) && fs.existsSync(genScript)) {
		...
		for (const args of [[genScript], [genScript, '--env', 'testco']]) {
			...
			if (r.status !== 0) {
				throw new Error(`Failed to generate wrangler config — scripts/gen-wrangler.mjs exited with ${r.status ?? 'signal'}`);
			}
		}
		return;
	}

	// ── Legacy fallback (templates without infra/env.prod) ──────────────────
	...
	if (api) {
		// `"name"` matches the FIRST occurrence only — the top-level worker name
		// (DO bindings use `"name"` too, but they appear later and must be kept).
		let out = api
			...
			// Producers are keyed by their BINDING so webhook and events stay
			// distinct. (A blanket `/"queue": "…"/g` replace collapsed both onto
			// one name, so the worker consumed its own webhook deliveries as
			// events — the bug this fixes.)
			.replace(/("binding"\s*:\s*"WEBHOOK_QUEUE"\s*,\s*"queue"\s*:\s*)"[^"]*"/, `$1"${safe}-webhook-delivery"`)
			.replace(/("binding"\s*:\s*"EVENTS"\s*,\s*"queue"\s*:\s*)"[^"]*"/, `$1"${safe}-events"`)
			.replace(/"dataset"\s*:\s*"[^"]*"/, `"dataset": "${safe}_api_requests"`)
			.replace(/"ADMIN_USERNAME"\s*:\s*"[^"]*"/, `"ADMIN_USERNAME": "${adminEmail}"`);
		// Consumers carry the same queue names but no binding to key on; the
		// producers are already renamed, so the remaining `"queue"` values (in
		// webhook-then-events order) are the two consumers.
		let consumer = 0;
		out = out.replace(/"queue"\s*:\s*"[^"]*"/g, (match) => {
			if (match.includes(`${safe}-`)) return match; // already a renamed producer
			consumer += 1;
			return `"queue": "${consumer === 1 ? `${safe}-webhook-delivery` : `${safe}-events`}"`;
		});
		let dlq = 0;
		out = out.replace(/"dead_letter_queue"\s*:\s*"[^"]*"/g, () => {
			dlq += 1;
			return `"dead_letter_queue": "${dlq === 1 ? `${safe}-webhook-dlq` : `${safe}-events-dlq`}"`;
		});
...
/**
 * Rewrite the resource values in `infra/env.prod` (the SSOT). Line-anchored
...
function personalizeInfraEnv(envPath: string, safe: string, dbName: string, placeholderId: string, adminEmail: string): void {
	...
	assign('WORKER_API', `${safe}-api`);
	assign('WORKER_STUDIO', `${safe}-studio`);
	// No custom domain for a fresh project — the generator omits the routes block.
	assign('STUDIO_DOMAIN', '');
	assign('STUDIO_D1_NAME', `${safe}-studio-db`);
	assign('STUDIO_D1_ID', placeholderId);
	assign('D1_NAME', dbName);
	assign('D1_ID', placeholderId);
	assign('D1_PREVIEW_ID', '');
	assign('R2_BUCKET', `${safe}-media`);
	assign('QUEUE_WEBHOOK', `${safe}-webhook-delivery`);
	assign('QUEUE_WEBHOOK_DLQ', `${safe}-webhook-dlq`);
	assign('QUEUE_EVENTS', `${safe}-events`);
...
function writeAdminEnv(destRoot: string, admin: AdminCredentials): void {
	...
	// ADMIN_USERNAME in the tracked config is set by `writeWranglerConfig`
	// (via infra/env.prod + regeneration, or a direct rewrite for legacy
	// templates) — deliberately NOT patched here, so the generated configs never
	// desync from the SSOT.
...
export async function initProject(
	...
	// D1 database name + bootstrap superadmin — resolved BEFORE the configs so
	...
	console.log('  pnpm dev                      # API :8788 · Studio :5174');
	...
	console.log(pc.dim('  Login at http://localhost:5174 after `pnpm dev`. Rotate the password in production.'));
